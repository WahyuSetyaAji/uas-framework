<x-guest-layout>
    ...
    <section x-data="{
        ...
        <template x-for="(img, index) in images" :key="index">
            <div class="absolute inset-0 transition-all duration-700 bg-center bg-cover" x-show="current === index"
                x-transition.opacity :style="`background-image: url('${img}')`"></div>
        </template>
        ...
        <div class="absolute z-10 flex justify-center w-full space-x-3 bottom-10">
            <template x-for="(img, index) in images" :key="index">
                <button class="w-4 h-4 border-2 border-white rounded-full"
                    :class="current === index ? 'bg-white' : 'bg-transparent'" @click="current = index"></button>
            </template>
        </div>
    </section>
    ...
    ===============================-->
    <section class="py-24 bg-gradient-to-b from-gray-100 via-white to-white">
        <div class="grid items-center max-w-6xl gap-14 px-6 mx-auto md:grid-cols-2">
            ...
            <div class="space-y-6">
                ...
                <p class="text-lg text-gray-700">
                    Mulai dari jok motor hingga mobil keluarga, semua dikerjakan
                    dengan presisi untuk kenyamanan terbaik Anda.
                </p>

            </div>

            <div>
                <img src="{{ asset('images/galeri/background/bowojok.webp') }}" alt="Tentang Bowo Jok"
    ...
    <section class="py-24 bg-gray-100">
        <div class="max-w-6xl px-6 mx-auto">
            ...
            @if ($produk->isEmpty())
                <p class="text-center text-gray-500">Belum ada produk untuk ditampilkan di galeri.</p>
            @endif

            <div class="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">

                {{-- Menggunakan variabel $produk dari HomeController --}}
                @foreach ($produk as $index => $item)
                    <div class="rounded-2xl overflow-hidden shadow-xl cursor-pointer transform hover:scale-[1.03] transition duration-300 group relative"
                        onclick="openModal('{{ $index }}')">

                        <img src="{{ asset($item->gambar_produk) }}"  alt="{{ $item->nama_produk }}"
                            class="object-cover w-full h-56 sm:h-56 md:h-64 transition duration-300 group-hover:opacity-90">

                        {{-- Overlay nama produk untuk estetika --}}
                        <div
                            class="absolute inset-0 bg-black/30 group-hover:bg-black/10 transition duration-300 flex items-end p-4">
                            <p class="text-white font-semibold text-lg drop-shadow-md">{{ $item->nama_produk }}</p>
                        </div>
                        </div>
                @endforeach
            ...
            <div class="mt-12 text-center">
                <a href="{{ route('produk.index') }}"
                    class="inline-block px-8 py-3 text-lg font-semibold transition bg-blue-900 text-white shadow-xl hover:bg-blue-800 rounded-xl">
                    Lihat Semua Produk Lainnya
                </a>
            </div>
    ...
    {{-- KODE JAVASCRIPT --}}
    <script>
        // Meneruskan data $produk (galeri) dari Controller ke JavaScript
        const galeri = @json($produk->toArray());

        function openModal(index) {
            const item = galeri[index];
            // Menggunakan item.gambar_produk yang sudah berisi path 'uploads/...'
            document.getElementById('modal-img').src = "{{ asset('') }}" + item.gambar_produk;
    ...
    <section class="py-24 bg-white">
            ...
            @else
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 place-items-center">
                    ...
                    @foreach ($testimoni as $index => $item)
                        ...
                        <div class="relative pb-8 w-full max-w-sm cursor-pointer"
                            onclick="openTestimoni({{ $index }})">
                            <!-- Main Card -->
                            <div
                                class="bg-white rounded-2xl p-6 border-t-4 {{ $color['border'] }} shadow-lg relative transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
                            ...
                            <!-- Avatar Circle -->
                            <div
                                class="absolute -bottom-6 -right-6 w-24 h-24 {{ $color['bg'] }} rounded-full border-4 border-white shadow-lg flex items-center justify-center">
        ...
        <div id="testimoniModal"
            ...
            onclick="closeTestimoni()">
            <div class="relative max-w-2xl w-full" onclick="event.stopPropagation()">
                <!-- Modal Content -->
                <div id="modalContent"
                    class="bg-white rounded-3xl p-10 border-t-8 shadow-2xl relative transform transition-all duration-500 scale-0">

                    <!-- Close Button -->
                    <button onclick="closeTestimoni()"
                        class="absolute top-4 right-4 w-10 h-10 bg-gray-200 hover:bg-gray-300 rounded-full flex items-center justify-center transition-colors">
                    ...
                    <div id="modalAvatar"
                        class="absolute -bottom-10 -right-10 w-32 h-32 rounded-full border-4 border-white shadow-2xl flex items-center justify-center">
        ...
        <script>
            ...
            function openTestimoni(index) {
                ...
                document.getElementById('modalDate').textContent = new Date(item.tanggal_testimoni).toLocaleDateString(
                'id-ID', {
                    day: 'numeric',
                    month: 'long',
                    year: 'numeric'
                ...
                modalContent.className =
                    `bg-white rounded-3xl p-10 border-t-8 ${color.border} shadow-2xl relative transform transition-all duration-500 scale-100`;
                ...
                document.getElementById('modalAvatar').className =
                    `absolute -bottom-10 -right-10 w-32 h-32 ${avatarColors[color.border]} rounded-full border-4 border-white shadow-2xl flex items-center justify-center`;
