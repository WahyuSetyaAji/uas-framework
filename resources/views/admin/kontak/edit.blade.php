                ...
                <form action="{{ route('admin.kontak.update', $kontak->id) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="email" name="email_kontak" id="email_kontak"
                            class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                            value="{{ old('email_kontak', $kontak->email_kontak) }}" required>
                        @error('email_kontak')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="no_kontak" id="no_kontak"
                            class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                            value="{{ old('no_kontak', $kontak->no_kontak) }}" required>
                        @error('no_kontak')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <select name="tipe" id="tipe"
                            class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                            required>
                            <option value="cs" {{ old('tipe', $kontak->tipe) == 'cs' ? 'selected' : '' }}>Customer Service</option>
                            <option value="order" {{ old('tipe', $kontak->tipe) == 'order' ? 'selected' : '' }}>Order</option>
                        </select>
                        @error('tipe')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
